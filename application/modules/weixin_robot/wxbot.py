...
class WXBot:
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            ...
            if self.DEBUG:
                ...
                logging.debug( '    | content: %s' % msg.get('content')[:20])
                logging.debug( '    --------------------------')
            ...
            try:
                ...
                if retcode == '1100':  # 从微信客户端上登出
                    ...
                    if selector == '2':  # 有新消息
                        ...
                        logging.debug( '[DEBUG] sync_check:', retcode, selector)
                        r = self.sync()
                        if r is not None:
                            self.handle_msg(r)
                else:
                ...
                logging.debug( '[ERROR] Except in proc_msg')
    ...
    def upload_media(self, fpath, is_img=False):
        if not os.path.exists(fpath):
            logging.debug( '[ERROR] File ' + fpath + ' not exists.')
            return None
        ...
        try:
            ...
            if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                ...
                logging.debug(r.text)
            if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                logging.debug( '[ERROR] Upload media failure.')
                return None
    ...
    def send_img_msg_by_uid(self, fpath, uid):
        ...
        if mid is None:
            logging.error("post media failed ,mid is null")
            return False
    ...
    def run(self):
        ...
        if self.login():
            ...
            self.record_status({"uin": self.uin, "status":-1 ,"desc":"login failed"})
            logging.error( '[ERROR] Web WeChat login failed .')
            return

            logging.info( '[INFO] begin to remove qrcode: qr_' +  self.uuid + '.png')
    ...
    def record_status(self,arr):
        with open(self.base_dir + '/account_list.json', 'r') as f:
            ...
            for i in range(len(users)):
                user = users[i]
                if "uuid" in user and user['uuid'] == self.uuid:
                    find = 1
                    users[i] = arr
            if find == 0:
                users.append(arr)
    ...
    def gen_qr_code(self, qr_file_path):
        ...
        if self.conf['qr'] == 'png':
            ...
            logging.debug(qr.terminal(quiet_zone=1))
    ...
    def wait4login(self):
        ...
        while retry_time > 0:
            ...
            if code == SCANED:
                ...
                logging.error( '[ERROR] WeChat login timeout. retry in %s secs later...' % (try_later_secs,))
    ...
    def login(self):
        if len(self.redirect_uri) < 4:
            logging.error( '[ERROR] Login failed due to network problem, please try again.')
            return False
