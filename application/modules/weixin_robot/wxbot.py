...
class WXBot:
    ...
    def __del__(self):
        if self.uin:
            self.record_status({"uin":self.uin,"status":0,"desc":"logout"})
    ...
    def get_contact(self):
        ...
        r.encoding = 'utf-8'
        if not os.path.exists(self.base_dir + "/robots/" + str(self.my_account['Uin'])):
            os.mkdir(self.base_dir +"/robots/" + str(self.my_account['Uin']))
            os.chmod(self.base_dir +"/robots/" + str(self.my_account['Uin']),0777)
        if self.DEBUG:
            with open(self.base_dir +"/robots/" + str(self.my_account['Uin']) + '/contacts.json', 'w') as f:
        ...
        if self.DEBUG:
            with open(self.base_dir +"/robots/" + str(self.my_account['Uin']) + '/contact_list.json', 'w') as f:
            ...
            with open(self.base_dir +"/robots/" + str(self.my_account['Uin']) + '/account_info.json', 'w') as f:
                f.write(json.dumps(self.my_account))
        return True
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            ...
            if self.DEBUG:
                ...
                logger.debug( '    | content: %s' % msg.get('content')[:20])
                logger.debug( '    --------------------------')
        ...
        while True:
            ...
            try:
                ...
                if retcode == '1100':  # 从微信客户端上登出
                    self.record_status({"uin":self.uin,"nickname":self.my_account['NickName'],"status":0,"desc":"exit"})
                    break
                    ...
                    if selector == '2':  # 有新消息
                        ...
                        logger.debug( '[DEBUG] sync_check:', retcode, selector)
                        r = self.sync()
                        if r is not None:
                            self.handle_msg(r)
                else:
                ...
                logger.error('[ERROR] Except in proc_msg')
                logger.exception('exception')
            check_time = time.time() - check_time
            if check_time < 0.8:
                time.sleep(1 - check_time)

    def verify_user(self,user,ticket):
        url_notify = self.base_uri + '/webwxstatusnotify'
        msg_id = str(int(time.time() * 1000)) + str(random.random())[:5].replace('.', '')
        url_verify = self.base_uri + '/webwxverifyuser?r='+ str(int(time.time() * 1000)) + str(random.random())[:5].replace('.', '')
        params = {
            'BaseRequest': self.base_request,
            "Code": 1,
        ...
        try:
            r = self.session.post(url_notify, data=data, headers=headers)
            logger.debug(r)
        ...
        params = {
            ...
            "SceneListCount":1,
            "VerifyContent":"",
            "VerifyUserList":[{"Value":user,"VerifyUserTicket":ticket}],
        ...
        data = json.dumps(params, ensure_ascii=False).encode('utf8')
        try:
            r = self.session.post(url_verify, data=data, headers=headers)
            logger.debug(r)
        except (ConnectionError, ReadTimeout):
            return False
        dic = r.json()
    ...
    def upload_media(self, fpath, uid,is_img=False):
        if not os.path.exists(fpath):
            logger.debug( '[ERROR] File ' + fpath + ' not exists.')
            return None
        ...
        #m = MultipartEncoder(files, boundary='----WebKitFormBoundaryZSy2TrWWLgrz3AyA')
        try:
            #r = requests.post(url_1, headers={'Content-Type': m.content_type}, data=m)
            ...
            if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                ...
                logger.debug(r.text)
            if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                logger.info("upload media twice failed,try py2j server.")
                gateway = JavaGateway()
                app = gateway.entry_point
                text = app.uploadFileToUrl(url_1, fpath,self.my_account['UserName'],uid,self.uin,self.sid,self.skey,self.device_id,self.session.cookies['webwx_data_ticket'],self.pass_ticket)
                mid = json.loads(text)['MediaId']
                if mid:
                    return mid
                else :
                    text = app.uploadFileToUrl(url_2, fpath, self.my_account['UserName'], uid, self.uin, self.sid,
                                               self.skey, self.device_id, self.session.cookies['webwx_data_ticket'],
                                               self.pass_ticket)
                    mid = json.loads(text)['MediaId']
                if mid:
                    return mid
    ...
    def send_file_msg_by_uid(self, fpath, uid):
        mid = self.upload_media(fpath,uid)
    ...
    def run(self):
        self.get_uuid()
        self.record_status({"status": 1, "desc": "wait for scan", "pid": os.getpid(), "uuid": self.uuid})
        self.gen_qr_code(self.base_dir + "/qr_" + self.uuid + ".png")
        logger.info( '[INFO] Please use WeChat to scan the QR code .')

        result = self.wait4login()

        if result != SUCCESS:
            logger.error( '[ERROR] Web WeChat login failed. failed code=%s' % (result,))
            return

        if self.login():
            self.record_status({"uin":self.uin,"status":1,"desc":"login succeed","pid":os.getpid(),"uuid":self.uuid})
            logger.info( '[INFO] Web WeChat login succeed .')
            os.remove(self.base_dir +'/qr_' + self.uuid + '.png')
        else:
            self.record_status({"uuid": self.uuid, "status":-1 ,"desc":"login failed"})
            logger.error( '[ERROR] Web WeChat login failed .')
            os.remove(self.base_dir +'/qr_' + self.uuid + '.png')
            return

        if self.init():
            self.record_status({"uin": self.uin, "status": 1, "desc": "init succeed","pid":os.getpid(),"uuid":self.uuid})
            logger.info( '[INFO] Web WeChat init succeed .')
        else:
            self.record_status({"uin": self.uin, "status": -1, "desc": "init failed"})
            logger.info( '[INFO] Web WeChat init failed')
            return
        self.status_notify()
        self.get_contact()
    ...
    def wait4login(self):
        ...
        while retry_time > 0:
            ...
            if code == SCANED:
                logger.info( '[INFO] Please confirm to login .')
                tip = 0
                ...
                logger.error( '[ERROR] WeChat login timeout. retry in %s secs later...' % (try_later_secs,))

                tip = 1  # 重置
                retry_time -= 1
                time.sleep(try_later_secs)
            else:
                logger.error( ('[ERROR] WeChat login exception return_code=%s. retry in %s secs later...' %
                       (code, try_later_secs)))
                tip = 1
                retry_time -= 1
                time.sleep(try_later_secs)
    ...
    def login(self):
        if len(self.redirect_uri) < 4:
            logger.error( '[ERROR] Login failed due to network problem, please try again.')
            return False
    ...
    def init(self):
        ...
        if not os.path.exists(self.base_dir + "/robots/" + self.uin) :
            os.mkdir(self.base_dir + "/robots/" + self.uin)
            os.chmod(self.base_dir + "/robots/" + self.uin,0777)
        self.get_icon(self.my_account['UserName'])
        return dic['BaseResponse']['Ret'] == 0
    ...
    def get_icon(self, uid, gid=None):
        ...
        data = r.content
        fn = self.base_dir +"/robots/" + str(self.my_account['Uin']) + '/icon_' + uid + '.jpg'
        if uid == self.my_account['UserName']:
            fn = self.base_dir +"/robots/" + str(self.my_account['Uin']) + '/head.jpg'
    ...
    def get_head_img(self, uid):
        ...
        r = self.session.get(url)
        data = r.content
        fn = self.base_dir +"/robots/" + str(self.my_account['Uin']) + '/head_' + uid + '.jpg'
        with open(fn, 'wb') as f:
            f.write(data)
        return fn
    ...
    def get_msg_img(self, msgid):
        ...
        r = self.session.get(url)
        data = r.content
        fn = self.base_dir + "/robots/" + str(self.my_account['Uin']) + '/img_' + msgid + '.jpg'
        with open(fn, 'wb') as f:
            f.write(data)
        return fn
    ...
    def get_voice(self, msgid):
        ...
        r = self.session.get(url)
        data = r.content
        fn = self.base_dir +"/robots/" + str(self.my_account['Uin']) + '/voice_' + msgid + '.mp3'
        with open(fn, 'wb') as f:
            f.write(data)
        return fn
