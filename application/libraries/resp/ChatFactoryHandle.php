...
class ChatFactoryHandle extends ResponseHandle {
    ...
    function match($fromUserName,$toUserName) {
        ...
        if ($match && $match['id']) {
            ...
            error_log("4,".json_encode($match_waiting));
            if ($match_waiting != null) {
                ...
                //匹配成功
                $result = "** 有人跟你匹配成功,可以开始聊天了.支持文字,图片,语音,位置信息发送.**";
                //给对方发送信息
                ...
                $r = $wechat->sendCustomMessage($number['app_id'],$number['secretkey'],$msg);

                //增加两条keywords
                $this->load->model("Keywords_model","key");
                $keyword1 = array("app_id"=>$number['id'],"type"=>2,"content"=>"resp/ChatToHandle","event"=>"chat","username"=>$fromUserName);
                $keyword2 = array("app_id"=>$number['id'],"type"=>2,"content"=>"resp/ChatToHandle","event"=>"chat","username"=>$another['user_open_id']);
    ...
    public function quit($fromUserName,$toUserName) {
        ...
        if ($matched && $matched['id']) {
            ...
            $msg = sprintf(kf_txt_msg,$another['user_open_id'],$result);
            $wechat->sendCustomMessage($number['app_id'],$number['secretkey'],$msg);
