...
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$this->ci->config->item('key');
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $this->ci->config->item('sslcert_path'));
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $this->ci->config->item('sslkey_path'));
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = $this->ci->config->item('appid');//公众账号ID
	   	$this->parameters["mch_id"] = $this->ci->config->item('mchid');//商户号
...
{	
	function __construct() 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = $this->ci->config->item('curl_timeout');
	}
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $this->ci->config->item('appid');//公众账号ID
		   	$this->returnParameters["mch_id"] = $this->ci->config->item('mchid');//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = $this->ci->config->item('appid');
	...
	{
		$urlObj["appid"] = $this->ci->config->item('appid');
		$urlObj["secret"] = $this->ci->config->item('secretkey');
	...
	{
		$jsApiObj["appId"] = $this->ci->config->item('appid');
		$timeStamp = time();
